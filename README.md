...
Now that every artist is mapped to a genre, I looped back through each week, summed plays by genre and found each genre's share of said week's total listening, creating a panel of one row per genre per week. I exported this panel to a .csv file (UglyDF in "Data" folder) so that I had a jumping off point when working within the same week, and for visualizing and regressing my data. 
